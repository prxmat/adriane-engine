...
10. **Phase 10 — secrets redaction + no-log.** ✅ **Done** ([ADR 0032](0032-secrets-redaction-and-no-log.md)). A deterministic in-engine `RegexSecretsRedactor` (governed, always-on, mask-default + `ADRIANE_SECRETS_POLICY=block` opt-in via `LlmError::SecretsBlocked`) pushed **before** PII (+ optional external `ADRIANE_SECRETS_REDACTOR_URL` augmentation), and a `ChannelDefinition.noLog` flag masked at the four `NodeCompleted` emit sites (sentinel `[REDACTED_NO_LOG]`) while **checkpointed in full** (durability ≠ observability). Known gap: free-text `NodeFailed.error`/`RunFailed.error`/`RunSuspended.reason` not covered.
