...
9. **Phase 9 — multimodal.** 🟡 **Foundation shipped** ([ADR 0030](0030-multimodal-input.md)). Additive `content_blocks` (text/image/audio/file) + `MediaSource` (artifact/url/base64) on `LlmMessage`/`LlmResponse` (text path byte-identical), per-adapter input fan-out (Anthropic/OpenAI/Gemini, image base64+url tested), PII-redaction of text blocks, the run entry surface (`inputBlocksChannel` → multimodal seed, 9e), and ArtifactRef bytes resolution at the gateway boundary (`MediaResolver`, 9c, with an inline-size cap). **Follow-ups**: TS parity (9f), output parsing (9out).
