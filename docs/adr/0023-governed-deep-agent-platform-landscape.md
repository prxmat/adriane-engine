...
9. **Phase 9 — multimodal.** 🟡 **Foundation shipped** ([ADR 0030](0030-multimodal-input.md)). Additive `content_blocks` (text/image/audio/file) + `MediaSource` (artifact/url/base64) on `LlmMessage`/`LlmResponse` (text path byte-identical), per-adapter input fan-out (Anthropic/OpenAI/Gemini, image base64+url tested), and PII-redaction of text blocks. **Follow-ups**: `MediaResolver`/ArtifactRef resolution (9c), the run-level entry field (9e), TS parity (9f), output parsing (9out).
