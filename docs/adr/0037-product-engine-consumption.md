...
- Status: **Proposed** (awaiting sign-off — widens the **public governance/storage surface** of `@adriane-ai/graph-sdk`, sets the **open-core publish boundary**, and changes how the private product builds. The two consequential forks were owner-confirmed via decision prompt; this ADR records the full model. Public engine API + open-core boundary → mandatory human review).
