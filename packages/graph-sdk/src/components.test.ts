...
/**
 * Cross-engine **fidelity** for the new pure components: build the *same* component
 * graph, run it once on the TS-fallback handler and once on the native Rust handler,
...
describeIfRust("@adriane/graph-sdk — new components TS-vs-Rust fidelity", () => {
  ...
  /** Run `build()` once per engine and return the value of `channel` on each. */
  const onEachEngine = async (
    build: () => Promise<Record<string, unknown>>,
    channel: string
  ): Promise<{ ts: unknown; rust: unknown }> => {
    process.env.ADRIANE_SDK_ENGINE = "ts";
    const ts = (await build())[channel];
    process.env.ADRIANE_SDK_ENGINE = "rust";
    const rust = (await build())[channel];
    return { ts, rust };
  };

  it("documentSplitter produces the same chunks on TS and Rust", async () => {
    const run = async () => {
      const app = createGraph({ name: "split-fid" })
        .channel("doc", { type: "string", default: "" })
        .channel("chunks", { type: "json", default: [] })
        .component(
          "split",
        ...
        .compile();
      const result = await app.run({ doc: "abcdefgh" }, { runId: `run_split_${process.env.ADRIANE_SDK_ENGINE}` as never });
      return result.channels as Record<string, unknown>;
    };
    const { ts, rust } = await onEachEngine(run, "chunks");
    expect(ts).toEqual(["abcd", "defg", "gh"]);
    expect(rust).toEqual(ts);
  });

  it("csvParser produces the same row objects on TS and Rust", async () => {
    const run = async () => {
      const app = createGraph({ name: "csv-fid" })
        .channel("csv", { type: "string", default: "" })
        .channel("rows", { type: "json", default: [] })
        .component("csv", components.csvParser({ from: "csv", into: "rows" }))
        .compile();
      const result = await app.run({ csv: "name,age\nAda,36\nBob,40" }, { runId: `run_csv_${process.env.ADRIANE_SDK_ENGINE}` as never });
      return result.channels as Record<string, unknown>;
    };
    const { ts, rust } = await onEachEngine(run, "rows");
    expect(ts).toEqual([
      { name: "Ada", age: "36" },
      { name: "Bob", age: "40" }
    ]);
  ...
  it("answerBuilder produces the same assembled answer on TS and Rust", async () => {
    const run = async () => {
      const app = createGraph({ name: "answer-fid" })
        .channel("answer", { type: "string", default: "" })
        .channel("ctx", { type: "json", default: [] as unknown[] })
        .channel("final", { type: "string", default: "" })
        .component("build", components.answerBuilder({ from: "answer", contextFrom: "ctx", into: "final" }))
        .compile();
      const result = await app.run(
        {
          answer: "The sky is blue.",
        ...
        { runId: `run_answer_${process.env.ADRIANE_SDK_ENGINE}` as never }
      );
      return result.channels as Record<string, unknown>;
    };
    const { ts, rust } = await onEachEngine(run, "final");
    expect(ts).toBe(
      "The sky is blue.\n\nSources:\n[1] d1: Rayleigh scattering.\n[2] d2: Sunlight is white."
    );
    expect(rust).toEqual(ts);
  ...
  it("fieldExtractor reduces a reasoning trace to its final answer on TS and Rust", async () => {
    const run = async () => {
      const app = createGraph({ name: "extract-fid" })
        .channel("ragResult", { type: "json", default: null as unknown })
        .channel("finalAnswer", { type: "string", default: "" })
        .component(
          "extract",
        ...
        .compile();
      const result = await app.run(
        { ragResult: { reasoning: "thought: x\nfinal:Adriane resumes from the latest checkpoint." } as unknown },
        { runId: `run_extract_${process.env.ADRIANE_SDK_ENGINE}` as never }
      );
      return result.channels as Record<string, unknown>;
    };
    const { ts, rust } = await onEachEngine(run, "finalAnswer");
    expect(ts).toBe("Adriane resumes from the latest checkpoint.");
    expect(rust).toEqual(ts);
  });
});
