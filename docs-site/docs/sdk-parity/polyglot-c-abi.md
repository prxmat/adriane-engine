...
The first contract is JSON/YAML in and JSON out:
...
remains on PyO3 for wheel ergonomics. The C ABI is the default for new callback-neutral SDK
surfaces.

## Current scope

The current ABI covers validation, DSL compilation, model policy, catalogs, native component
runs, and prebuilt-agent runs. Full custom-node runtime execution is intentionally tracked as
the next bridge layer because it needs host callbacks for handlers, predicates, streaming
events, and checkpointer lifecycle hooks.
